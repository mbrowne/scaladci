...
Version 7a with explicit role contracts
...
class ShoppingCart7 extends Specification {
  ...
  class PlaceOrder(company: Company, customer: Person) {
    ...
    role warehouse {
      type Contract = {stock: mutable.Map[Int, Product]}
    ...
    role customer {
      type Contract = {var cash: Int, owns: mutable.Map[Int, Product]}
    ...
    role cart {
      type Contract = {items: mutable.Map[Int, Product]}
    ...
    role companyAccount {
      type Contract = {var bankAccount: Int}
      
      def depositFunds(amount: Int) { this.bankAccount += amount }
    }
  }
